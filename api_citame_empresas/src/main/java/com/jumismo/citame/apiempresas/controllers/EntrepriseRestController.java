...
public class EntrepriseRestController {
	 ...
	 */
	@ApiOperation(value = "Update an entreprise", response = EntrepriseDTO.class)
	@ApiResponses({
		@ApiResponse(code = 200, message = "Entreprise updated"),
		@ApiResponse(code = 404, message = "Entreprise not found"),
		@ApiResponse(code = 500, message = "Internal Server Error")
	})
	@PutMapping("/entreprises/{id}")
	public EntrepriseDTO update(@PathVariable Long id, @RequestBody EntrepriseDTO empresa) {
		return entrepriseService.update(id, empresa);
	}
